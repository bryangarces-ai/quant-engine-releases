...
1. Download **`ZentheaQuant-v1.0.0-Windows.zip`** from the link above.
...
2. Download the latest `ZentheaQuant-vX.X.X-Windows.zip` or `ZentheaQuant.exe`.
...
1. **$0 Live Capital Rule:** Strict lockout prevents unauthorized real-money execution.
2. **1.0% Institutional Risk Sizing:** Automated mathematical position sizing capped at 1% portfolio risk.
3. **ISP-Resilient Exchange Connectivity:** Built-in connection layer resilient against local ISP DNS/firewall restrictions.
4. **6-Gate Pre-Flight Strategy Certification:** Profit factor $\ge 1.50$, Max Drawdown $< 10\%$, Sharpe $\ge 0.50$, OOS profitability, Monte Carlo 95th percentile, and 0.00% Probability of Ruin.
